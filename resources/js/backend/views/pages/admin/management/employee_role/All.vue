    ...
    <div>
        <div class="card rounded-none">
            <div class="card-header pt-3 pb-1 d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="bn" style="font-size: 22px !important;">All employee role</h4>
                </div>
                <div class="d-flex justify-content-between">
                    ...
                    <div class="ps-3"><router-link :to="{ name: `EmployeeRoleCreate` }" class="btn btn-sm btn-info"> Create
                        </router-link></div>
                </div>
            </div>
            <div class="card-body px-4 py-2 form_area custom_scroll">
                <div class="custom_table nowrap table-responsive w-100 h-100 custom_scroll">
                    <table class="table table-bordered table-hover text-center">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in all_role_category?.data" :key="item.id">
                                ...
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-3">
                                        <router-link :to="{ name: `EmployeeRoleEdit`, params: { id: item.id } }"
                                            class="btn btn-sm btn-outline-info"> Edit </router-link>
                                        <a @click.prevent="role_category_delete(item.id)" href="#"
...
import { employee_role_store } from './setup/store';
export default {
    created: function () {
        this.fetch_all_role_category();
    },
    methods: {
        ...mapActions(employee_role_store, {
            fetch_all_role_category: 'all',
            role_category_delete: 'delete',
        })
    },
    computed: {
        ...mapState(employee_role_store, {
            all_role_category: 'all_data'
        })
    },
}
</script>
