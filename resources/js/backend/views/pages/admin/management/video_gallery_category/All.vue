    ...
    <div>
        <div class="card rounded-none">
            <div class="card-header pt-3 pb-1 d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="bn" style="font-size: 22px !important;">All video gallery category</h4>
                </div>
                <div class="d-flex justify-content-between">
                    ...
                    <div class="ps-3"><router-link :to="{ name: `VideoGalleryCategoryCreate` }" class="btn btn-sm btn-info">
                            Create
                        </router-link></div>
                </div>
            </div>
                            ...
                            <tr v-for="(item, index) in all_data?.data" :key="item.id">
                                ...
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-3">
                                        <router-link :to="{ name: `VideoGalleryCategoryEdit`, params: { id: item.id } }"
                                            class="btn btn-sm btn-outline-info"> Edit </router-link>
                                        <a @click.prevent="category_delete(item.id)" href="#"
...
import { video_gallery_category_store } from './setup/store';
export default {
    created: function () {
        this.fetch_all_data();
    },
