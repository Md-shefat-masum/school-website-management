...
class NavbarMenuItemController extends Controller
{
    public function all()
    {
        ...
        $query = NavbarMenuItem::where('status', $status)->orderBy($orderBy, $orderByType);
    ...
    public function bulk_import()
    {
        ...
        foreach (request()->data as $item) {
            ...
            $item = (object) $item;
            $check = NavbarMenuItem::where('id', $item->id)->first();
            if (!$check) {
                try {
                    NavbarMenuItem::create((array) $item);
                } catch (\Throwable $th) {
                    return response()->json([
                        'err_message' => 'validation error',
