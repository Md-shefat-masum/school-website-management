...
class SettingsTitleValueController extends Controller
{
    public function all()
    {
        ...
        $query = SettingTitleValue::where('status', $status)->orderBy($orderBy, $orderByType);
    ...
    public function update()
    {
        ...
        $data = SettingTitleValue::find(request()->id);
    ...
    public function bulk_import()
    {
        ...
        foreach (request()->data as $item) {
            ...
            $item = (object) $item;
            $check = SettingTitleValue::where('id', $item->id)->first();
            if (!$check) {
                try {
                    SettingTitleValue::create((array) $item);
                } catch (\Throwable $th) {
                    return response()->json([
                        'err_message' => 'validation error',
